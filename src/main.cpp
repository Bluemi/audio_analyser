...
void tick(const AudioFile &file, const float time)
{
	const int loudness_left = (int)(file.getLoudness_sum(Time::fromSeconds((float)time, 44100), Time::fromSeconds(dur, 44100), StereoChannel::LEFT)*scale);
	const int loudness_right = (int)(file.getLoudness_sum(Time::fromSeconds((float)time, 44100), Time::fromSeconds(dur, 44100), StereoChannel::RIGHT)*scale);
	const int loudness_left2 = (int)(file.getLoudness_diff(Time::fromSeconds((float)time, 44100), Time::fromSeconds(dur, 44100), StereoChannel::LEFT)*scale);
	const int loudness_right2 = (int)(file.getLoudness_diff(Time::fromSeconds((float)time, 44100), Time::fromSeconds(dur, 44100), StereoChannel::RIGHT)*scale);
